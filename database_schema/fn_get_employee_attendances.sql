CREATE OR REPLACE FUNCTION public.fn_get_employee_attendances(
    ...
    RETURNS TABLE(
		...
		permisions_start_time time without time zone,
		permissions_end_time time without time zone,
   ...
   WITH CTE_Dates AS (
	...
	Sessions_Per_Day AS (
		SELECT
			...
			COUNT(DISTINCT leave_days.session) AS sessions_count
		FROM employee_applied_leave_days leave_days
		JOIN employee_applied_leaves l ON l.id = leave_days.applied_leave_request_id
		WHERE l.employee_id = p_employee_id AND leave_days.status <> 'Cancelled'
		GROUP BY leave_days.leave_date
	)
	SELECT 
		...
		CASE
		WHEN COALESCE(spd.sessions_count, 0) = 2 THEN 'Full Day'
		ELSE COALESCE(leave_days.session, '')
		END AS session,
		...
		COALESCE(spd.sessions_count, 0) AS sessions_count
	FROM CTE_Dates dat
	...
	LEFT JOIN Sessions_Per_Day spd
		ON spd.leave_date = dat.DateValue
